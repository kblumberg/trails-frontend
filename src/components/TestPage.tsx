...
const saveSlideMovement = async (address: string, trailheadId: number, step: number, slide: number, isForward: boolean, dispatch: Dispatch, data: IState) => {
    ...
    console.log(`saveSlideMovement response`);
...
const TestPage = (props: any) => {
    ...
    const className = slideNum == slides.length - 1 ? '' : 'hidden';
    ...
    const trailDivs = items.map((x, i) => {
        ...
        console.log(`step ${i} lastComplete = ${lastComplete} lastCompleteMinutesAgo = ${Math.floor(lastCompleteSecondsAgo / (60))}`);
        console.log(`${hours}:${minutes.toLocaleString('en-US', {
            minimumIntegerDigits: 2,
            useGrouping: false
          })}:${seconds.toLocaleString('en-US', {
            minimumIntegerDigits: 2,
    ...
    const lastComplete = data.xps.filter(x => x.trailheadId == curTrailheadId && x.step == step ).reduce((a, b) => Math.max(a, b.timestamp), 0);
    return (
        <>
            ...
            <Modal className='trails-modal' show={show} onHide={handleClose}>
                ...
                <Modal.Footer>
                    <div className='row'>
                        ...
                        <div className='col'>
                            <Button disabled={!completed && lastComplete == 0 && programIds && programIds.length > 0} variant="primary" onClick={() => {
                                const num = slideNum;
                                ...
                                if (num + 1 == slides.length) {
                                    ...
                                    if (num + 2 == slides.length) {
                                        console.log(`hasCompleted = ${hasCompleted.length}`)
                                        playComplete();
                                        successTimeline.play();
